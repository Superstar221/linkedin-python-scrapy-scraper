...
class LinkedInPeopleProfileSpider(scrapy.Spider):
    ...
    def __init__(self, input=None, profile=None):
    ...
    def parse_profile(self, response):
        ...
        item['name'] = summary_box.css("h1::text").get(default='').strip()
        item['description'] = summary_box.css("h2::text").get(default='').strip()

        ## Location
        try:
            item['location'] = summary_box.css('div.top-card__subline-item::text').get(default='').strip()
        except:
            item['location'] = summary_box.css('span.top-card__subline-item::text').get(default='').strip()
        ...
        for block in experience_blocks:
            ...
            ## description
            try:
                experience['description'] = block.css('p.show-more-less-text__text--more::text').get(default='').strip()
            except Exception as e:
